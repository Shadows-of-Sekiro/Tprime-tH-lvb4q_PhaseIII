...
class fatJetUncertaintiesProducer(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch("%s_mass_raw" % self.jetBranchName,
                        "F",
                        lenVar=self.lenVar)
        self.out.branch("%s_eta_raw" % self.jetBranchName,
                        "F",
                        lenVar=self.lenVar)
        self.out.branch("%s_phi_raw" % self.jetBranchName,
                        "F",
                        lenVar=self.lenVar)                        
        self.out.branch("%s_pt_clean" % self.jetBranchName,
                        "F",
                        lenVar=self.lenVar)   
        self.out.branch("%s_mass_clean" % self.jetBranchName,
                        "F",
                        lenVar=self.lenVar)                           
        self.out.branch("%s_eta_clean" % self.jetBranchName,
                        "F",
                        lenVar=self.lenVar)
        self.out.branch("%s_phi_clean" % self.jetBranchName,
                        "F",
                        lenVar=self.lenVar) 
	...
	# muion and electron objects are accessed here and check closeness with jets
        ...
        electrons = [x for x in allelectrons if x.pt > 40.  and ( abs(x.eta) < 2.5 ) and x.cutBased >=3] #loose pt cut for veto 

        muons     = [x for x in allmuons if x.pt > 40. and  abs(x.eta) < 2.1 and x.isGlobal and x.highPtId >= 2  ] # High pT ID muon is selected for cleaning
        ...
        closelepton1 = ROOT.TLorentzVector()
        closelepton2 = ROOT.TLorentzVector()        

	Hadronic     = False
	SemiLeptonic = False
	DiLeptonic   = False

        nlepton = len(muons) + len(electrons) 


        if (nlepton == 0) :  # There is no other loose electron or muon 
	        Hadronic=True


        if (nlepton == 1) :  # There is a electron or muon 
	   SemiLeptonic = True	
        
           if len(muons) == 0 :         
              closelepton1 = electrons[0].p4()

           else :
              closelepton1 = muons[0].p4()
           

        if (nlepton == 2) :  # There is a electron and muon 
        
            DiLeptonic = True	
            if len(muons) == 0  :        
            	 closelepton1 = electrons[0].p4()
            	 closelepton2 = electrons[1].p4()

            if len(electrons) == 0  :
               closelepton1 = muons[0].p4()
               closelepton2 = muons[1].p4()               

            if ( len(electrons) == 1 and len(muons) == 1):
               closelepton1 = muons[0].p4()
               closelepton2 = electrons[0].p4()
	    ...
	    #if (nlepton == 1) :			
	    #    dR = newjet.DeltaR(closelepton1) 
            #    print(" dr      = '%s'" % dR)       

	    #newpT = self.jetReCalibrator.getCorrectionARJUN(newjet, jet.area, rho)

	    #print(" Dr check	 = '%s'" % self.dRcheck)	
	    #print(" correctionk	 = '%s'" % jet_correction)	
	    ...
	    #jet_correction = jet_pt / jet_rawpt

            #jets_pt_raw.append(jet_rawpt)
            #jets_mass_raw.append(jet_rawmass)
            #jets_corr_JEC.append(jet_pt / jet_rawpt)
	    # Lepton cleaning applies from here and these values are calculated and then stored
            #   newjet vector is created for jet raw		
           
	    newjet.SetPtEtaPhiM( jet_rawpt, jet_eta, jet_phi, jet_rawmass)

            #if Hadronic:
            #jet_correction = self.jetReCalibrator.getCorrectionARJUN(newjet, jet.area, rho)

            ZeroEnergy = 1.0    
                
            if SemiLeptonic:

            	if newjet.DeltaR(closelepton1) < self.dRcheck:
            		 if  (newjet.E() > closelepton1.E()):
					newjet  -= closelepton1
            				#print(" After dR = '%s'" %  newjet.DeltaR(closelepton1))	 
            		 else :
            		 		ZeroEnergy = -1.0 		                
	        else :
        	   ZeroEnergy = 1.0 
            ...
            if DiLeptonic:
            	...
            	if newjet.DeltaR(closelepton1) < self.dRcheck:
            		 if  (newjet.E() > closelepton1.E()):
            				#print(" Before dR = '%s'" %  newjet.DeltaR(closelepton1))	 
            		 		newjet  -= closelepton1
            				#print(" After dR = '%s'" %  newjet.DeltaR(closelepton1))	 

            		 else :
            		 	ZeroEnergy = -1.0 
		if newjet.DeltaR(closelepton2) < self.dRcheck:            		
            		 if  ( newjet.E() > closelepton2.E() ):
            				#print(" Before dR = '%s'" %  newjet.DeltaR(closelepton2))	 
            		 		newjet  -= closelepton2
            				#print(" After dR = '%s'" %  newjet.DeltaR(closelepton2))	 
            		 else :
            		 	ZeroEnergy = -1.0 
           	else :
           	    ZeroEnergy = 1.0

            # Check whether  cleaning is required or lepton energy is greater than jet	    
            if ZeroEnergy == 1.0:
		       	jet_correction = self.jetReCalibrator.getCorrectionARJUN(newjet, jet.area, rho)        
            ...
            jets_pt_raw.append(newjet.Pt())
            jets_mass_raw.append(newjet.M())

            jets_eta_raw.append(newjet.Eta())
	    jets_phi_raw.append(newjet.Phi())
	    jets_pt_clean.append(newjet.Pt() * jet_correction )
	    jets_mass_clean.append(newjet.M() * jet_correction)
	    jets_eta_clean.append(newjet.Eta())
	    jets_phi_clean.append(newjet.Phi())        	    

            jets_corr_JEC.append(jet_correction)
		...
		# Calculate PuppiSD correction using cleaned pT and eta info
		jet_pt = newjet.Pt() * jet_correction
		jet_eta = newjet.Eta()

                puppisd_genCorr = self.puppisd_corrGEN.Eval(jet_pt)
                if abs(jet_eta) <= 1.3:
                    puppisd_recoCorr = self.puppisd_corrRECO_cen.Eval(jet_pt)
                else:
                    puppisd_recoCorr = self.puppisd_corrRECO_for.Eval(jet_pt)
        ...
        self.out.fillBranch("%s_eta_raw" % self.jetBranchName, jets_eta_raw)
        self.out.fillBranch("%s_phi_raw" % self.jetBranchName, jets_phi_raw)

        self.out.fillBranch("%s_pt_clean" % self.jetBranchName,  jets_pt_clean)
        self.out.fillBranch("%s_mass_clean" % self.jetBranchName,jets_mass_clean)
        self.out.fillBranch("%s_eta_clean" % self.jetBranchName, jets_eta_clean)
        self.out.fillBranch("%s_phi_clean" % self.jetBranchName, jets_phi_clean)
