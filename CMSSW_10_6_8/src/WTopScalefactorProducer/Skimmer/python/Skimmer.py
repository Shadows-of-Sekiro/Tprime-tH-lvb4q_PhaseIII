...
#         self.out.fillBranch("SelectedJet_tau41",tau41)
        ...
        for the Tprime decay to top and Higgs and final state consist of
	lepton + fatjet(higgs decayed jet) + bjet
        
        For the Tprime leptonic decay state we should ensure to have atleast one leptons,
	and one Fat jet .
	...
	eventLep  = 0
        ...
        for lep in muons :
            if lep.isGlobal and  lep.pt > 35.0 :
                eventMuons += 1
		eventLep   += 1
        for lep in electrons :
            if lep.cutBased >= 2 and lep.pt > 35.0 :
                eventElectrons += 1
		eventLep   += 1
        for jet in jets :
            if jet.pt > 25.0:
